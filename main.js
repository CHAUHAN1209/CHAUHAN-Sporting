...
/* ── CART: declared globally so onclick="addToCart()" in HTML works ── */
var CART_KEY = 'chauhanCart';

function addToCart(name, price, img = '', grade = 'Grade 1 English Willow') {
  ...
  if (existing) {
    ...
    cart.push({ id, name, price, img, grade, qty: 1 });
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (function initCartCount() {
    const cart = JSON.parse(localStorage.getItem('chauhanCart') || '[]');
  ...
  window.openBatModal = function(data) {
    ...
    modalEl.querySelector('.btn-modal-cart').onclick = () => {
      addToCart(data.name, data.price, data.img || '', data.grade || 'Grade 1 English Willow');
